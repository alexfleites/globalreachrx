...
*/
	...
	function check_status($val){
	$result='';
	if($val==1){$result.='<span style="color:green;">active</span>';}
	elseif($val==0){$result.='<span style="color:red;">inactive</span>';}
	return $result;
	}
...
function AjaxSearchSuggest($url){
	...
	/* check if patient have card created */
	function patient_have_card($id){

		$conn = mysql_connect("localhost", "lobalre3_printrx", "TI46Tl8@EP0{");
		$db = mysql_select_db("lobalre3_printrxcard");
